...
public:
    ...
    GAP_DecompApp(UtilParameters& util_param, const Instance& ins):
        DecompApp(util_param), ins_(ins)
    {
        initializeApp();
    }

    virtual ~GAP_DecompApp() { UtilDeleteMapPtr(models_); };
...
DecompSolverStatus GAP_DecompApp::solveRelaxed(
...
Cost dip(const Instance& ins, UtilParameters& utilParam)
{
   try {
      ...
      GAP_DecompApp gap(utilParam, ins);
