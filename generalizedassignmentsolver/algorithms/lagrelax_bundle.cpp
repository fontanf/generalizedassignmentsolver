...
double lagrelax_assignment_bundle_subproblem(
        const Instance& ins, const std::vector<double>& mu, std::vector<double>& grad)
{
    ...
    for (AgentIdx i=0; i<m; ++i) {
        ...
        for (ItemIdx j=0; j<n; ++j) {
            ...
            knapsacksolver::Profit p = std::ceil(mult * mu[j] - mult * a.c);
            if (p > 0) {
...
LagRelaxAssignmentBundleOutput generalizedassignmentsolver::lagrelax_assignment_bundle(const Instance& ins, Info info)
{
    /*
    ...
    for (Counter it = 0; it < 1000; ++it) {
        ...
        // Solve subproblem
        double l = lagrelax_assignment_bundle_subproblem(ins, mu, vec_elem);
        std::cout << l << std::endl;
...
LagRelaxKnapsackBundleOutput generalizedassignmentsolver::lagrelax_knapsack_bundle(const Instance& ins, Info info)
{
