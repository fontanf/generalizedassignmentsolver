...
Solution generalizedassignmentsolver::random_infeasible(const Instance& ins, std::mt19937_64& gen)
{
...
Output generalizedassignmentsolver::random(const Instance& ins, std::mt19937_64& gen, Info info)
{
    VER(info, "*** random ***" << std::endl);
    Output output(ins, info);

    Solution sol = random_infeasible(ins, gen);
